...
function PlayerPage() {
  ...
  // Sync a partial result to the leaderboard after each key is claimed
  const syncPartialResult = useCallback((p: Progress) => {
    if (ADMIN_TEST) return;
    if (!p.name || p.won.length === 0) return;
    const times = [0, 1, 2, 3].map((i) => p.times[`c${i + 1}`]?.seconds ?? null);
    const allDone = times.every((x) => x != null);
    const label = p.displayName || p.name;
    partialSubmit.mutate({ data: {
      playerName: p.name.trim().toLowerCase(),
      firstName: label.split(' ')[0] || label,
      lastName: label.split(' ').slice(1).join(' ') || '\u2014',
      score: p.score,
      rank: rankFor(p.score),
      timeC1: times[0], timeC2: times[1], timeC3: times[2], timeC4: times[3],
      totalTime: allDone ? times.reduce((a, x) => a + (x || 0), 0) : null,
      isTest: ADMIN_TEST,
    } });
  }, []); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    if (progress.won.length > prevWonLen.current || progress.submitted) {
      const newKeyWon = progress.won.length > prevWonLen.current;
      prevWonLen.current = progress.won.length;
      syncToServer(progress);
      if (newKeyWon) syncPartialResult(progress);
    }
