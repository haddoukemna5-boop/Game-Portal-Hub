...
function Finale({ progress, setProgress, onReset, passwordHash }: { progress: Progress; setProgress: (p: Progress) => void; onReset: () => void; passwordHash: string }) {
  ...
  const unlock = () => { if (opened || submit.isPending) return; if (code.trim().toUpperCase() !== 'RISK') { setError('Not quite. Shift U-L-V-N three places backward.'); return; } setError(''); const next = { ...progress, submitted: true }; const label = progress.displayName || progress.name; const payload: GameResultInput = { playerName: progress.name.trim().toLowerCase(), passwordHash, firstName: label.split(' ')[0] || label, lastName: label.split(' ').slice(1).join(' '), score: progress.score, rank: rankFor(progress.score), timeC1: times[0], timeC2: times[1], timeC3: times[2], timeC4: times[3], totalTime: total, isTest: PREVIEW }; submit.mutate({ data: payload }, { onSuccess: () => { setProgress(next); setOpened(true); void qc.invalidateQueries({ queryKey: getListResultsQueryKey() }); void qc.invalidateQueries({ queryKey: getGetResultsSummaryQueryKey() }); } }); };
